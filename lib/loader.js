...
var loader = {
  ...
  **/
  loadComponent: function (pagedata, componentId, context, locale) {
    ...
    component.text = coreutils.merge([component.text, component.data.text]);

    // Let's resolve all links related to this component
    component.links = coreutils.merge([component.links, component.data.links]);

    // Resolve style
    component.style = coreutils.merge([component.style, component.data.style]);

    // Resolve variables
    component.variables = coreutils.merge([component.variables, component.data.variables]);

    // Resolve images
    component.images = coreutils.merge([component.images, component.data.images]);
  ...
  findComponent: function (pagedata, componentId, context) {
    ...
    var componentRepo    = path.join(context.home.componentsDir, repoName);
    var componentDir     = path.join(componentRepo, parts[1] + "/" + parts[2]);
