...
var component = {
  ...
  theme: function (pagedata, componentId) {
    var globalTheme    = pagedata.global.theme;
    var pageTheme      = pagedata.theme;
    var component      = pagedata.components[componentId];
    var compTheme;

    if (component instanceof Object) {
      for (name in component) break;
      compTheme = component[name].theme;
    }

    return utils.merge(globalTheme, pageTheme, compTheme);
  },
  ...
  load: function (pagedata, componentId, context) {
    ...
    componentCopy['theme']     = this.theme(pagedata, componentId);
