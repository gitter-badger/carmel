...
context.dev = function() {
  return context.app.config.settings.development;
}

context.prod = function() {
  return context.app.config.settings.production;
}

context.getPages = function () {
  var pages = context.app.config.pages;
  var components = ['navbar', 'article', 'footer'];
  var hasTopBar = false;
  if (hasTopBar) {
    components.unshift('topbar');
  }
  var articlePage = {
      page: 'article',
      layout: 'default',
