    ...
    if (components && components.length > 0) {
      components.forEach(function(component) {
        ...
        if (fs.existsSync(jsFile)){
          ...
          imports = imports  + "var " + controllerName + " = require('components/" + pagedata.page + "/" + name + "/scripts')($, app, '" + (locale.default ? '' : locale.id) + "');\n";
          imports = imports + "app.controller('" + controllerName + "', ['$scope', '$http', '$location', '$sce' ," + controllerName + "]);\n";
        }
      });
    }
