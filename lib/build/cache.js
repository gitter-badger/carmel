  ...
  pageHash: function(context, locale, pagedata, type) {
    ...
    if (pagedata && pagedata.componentsData) {
        pagedata.componentsData.forEach(function(component) {
          ...
          if (pagedata.article) {
            var articleHash = cache.filehash(path.join(loader.articlesDistDir(locale), pagedata.article.id + ".json"));
            clearText += articleHash;
          }
        });
    }

    if (pagedata.extra) {
      clearText += JSON.stringify(pagedata.extra);
    }

    if (pagedata.site) {
      clearText += JSON.stringify(pagedata.site);
    }

    if (pagedata.menu) {
      clearText += JSON.stringify(pagedata.menu);
    }

    if (pagedata.article) {
      clearText += JSON.stringify(pagedata.article);
    }
